...
filament_diameter:
#   Номинальный диаметр необработанной нити (в мм) при ее поступлении в экструдер. 
#   Этот параметр должен быть указан.
#max_extrude_cross_section:
#   Максимальная площадь (в мм^2) поперечного сечения экструзии (например, ширина экструзии, 
#   умноженная на высоту слоя). Эта настройка предотвращает чрезмерное выдавливание при 
#   относительно небольших перемещениях XY.   Если перемещение запрашивает скорость выдавливания, 
#   которая превышает это значение, это приведет к возвращению ошибки. 
...
#   Сопротивление (в омах) тяги, присоединенной к термистору. Этот параметр действителен
